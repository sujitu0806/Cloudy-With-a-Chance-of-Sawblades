...
'''def umbrellaCollision(app, sawblade):
    distanceToUmbrella = distance(app.cx, app.cy, sawblade.xCoord, sawblade.yCoord)
    umbrellaRadius = 55

    if distanceToUmbrella < umbrellaRadius + sawblade.radius:
        # Calculate the normal vector at the collision point
        normalX = sawblade.xCoord - app.cx
        normalY = sawblade.yCoord - app.cy
        ...
        # Optional: Add a small displacement to prevent overlapping
        sawblade.xCoord += normalX
        sawblade.yCoord += normalY'''


def umbrellaCollision(app, sawblade):
    R = 50  # Radius of the umbrella arc
    cx, cy = app.cx, app.cy
    sx, sy = sawblade.xCoord, sawblade.yCoord

    # Calculate distance to umbrella center
    distance_to_center = distance(cx, cy, sx, sy)
    
    if distance_to_center < R + sawblade.radius and app.umbrellaCooldown <= 0:
        # Check if the line from sawblade to player intersects the back of the umbrella
        if sy < app.lineY1 + 10:
            if not canSee(sx, sy, cx, cy, app.lineX1, app.lineY1, app.lineX2, app.lineY2):
                # Calculate the normal vector at the collision point
                normal_x = sx - cx
                normal_y = sy - cy
                ...
                # Reflect the sawblade's velocity over the normal vector
                dot_product = sawblade.sawbladeDX * normal_x + sawblade.sawbladeDY * normal_y
                sawblade.sawbladeDX = sawblade.sawbladeDX - 2 * dot_product * normal_x
                sawblade.sawbladeDY = sawblade.sawbladeDY - 2 * dot_product * normal_y

                # Optional: Add a small displacement to prevent overlapping
                sawblade.xCoord += normal_x
                sawblade.yCoord += normal_y

                app.umbrellaCooldown = 20
