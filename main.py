...
def onAppStart(app):
    ...
    #Importing frames of cloud animations
    # Custom made on https://www.piskelapp.com/
    ...
    #Initializing frames, frame speeds, etc...
    ...
    #Initializing all game physics, states, and aesthetics
    ...
    app.score = 0
    app.scoreSize = 130
    app.scoreSlamTimer = 0
    app.scoreSlamActive = False
...
def redrawAll(app):
    ...
    #Creating a checkerboard background diagonal scrolling effect
    ...
    for row in range(app.numRows + 1):
            for col in range(app.numCols + 1):
                ...
                checkerRow = (row + app.offsetY // app.squareSize) % 2 
                checkerCol = (col + app.offsetX // app.squareSize) % 2

                if (checkerRow + checkerCol) % 2 == 0: 
    ...
    if app.isStarting == True:
        ...
        drawLabel("Press 'p' to Play!", app.width/2, 320 + 5, size=app.wordSet3Size, fill = 'black', bold = True, font='monospace', opacity = 60)
        drawLabel("Press 'p' to Play!", app.width/2, 320, size=app.wordSet3Size, fill = 'yellow', bold = True, font='monospace', border = 'black')
        drawLabel("Press 'i' for", app.width/2, 400 + 5, size=app.wordSet3Size, fill = 'black', bold = True, font='monospace', opacity = 60)
        drawLabel("Press 'i' for", app.width/2, 400, size=app.wordSet3Size, fill = 'yellow', bold = True, font='monospace', border = 'black')
        drawLabel("Instructions!", app.width/2, 450 + 5, size=app.wordSet3Size, fill = 'black', bold = True, font='monospace', opacity = 60)
        drawLabel("Instructions!", app.width/2, 450, size=app.wordSet3Size, fill = 'yellow', bold = True, font='monospace', border = 'black')
    ...
    #Menu Screen
        ...
        drawRect(0, 0, 400, 500, fill = 'lightCoral')

        drawImage('instructionsNew.png', 10, 10, width = 390, height = 480)
    ...
    #Game Screen
        ...
        drawLabel(app.score, app.width/2 + 8, 208, size=app.scoreSize, fill = 'grey', bold = True, font='monospace', opacity=60)
        drawLabel(app.score, app.width/2, 200, size=app.scoreSize, fill = 'red', bold = True, font='monospace', opacity=60, border = 'black')
        ...
        if app.umbrellaActive: 
            ...
            drawLine(app.lineX1, app.lineY1, app.lineX2, app.lineY2, rotateAngle=app.umbrellaAngle, fill = 'lightBlue')
    ...
    #End Screen
        ...
        drawLabel('Score', app.width/2 + 4, 82, size = 50, fill = 'grey', bold = True, font='monospace', opacity=80)
        drawLabel('Score', app.width/2, 78, size = 50, fill = 'red', bold = True, font='monospace', opacity=80, border = 'black')
        drawLabel(app.score, app.width/2 + 8, 198, size = 150, fill = 'grey', bold = True, font='monospace', opacity=80)
        drawLabel(app.score, app.width/2, 190, size = 150, fill = 'red', bold = True, font='monospace', opacity=80, border = 'black')
        drawLabel("Press 'p' to Play!", app.width/2, 320 + 5, size=app.wordSet3Size, fill = 'black', bold = True, font='monospace', opacity = 60)
        drawLabel("Press 'p' to Play!", app.width/2, 320, size=app.wordSet3Size, fill = 'yellow', bold = True, font='monospace', border = 'black')
        drawLabel("Press 'i' for", app.width/2, 400 + 5, size=app.wordSet3Size, fill = 'black', bold = True, font='monospace', opacity = 60)
        drawLabel("Press 'i' for", app.width/2, 400, size=app.wordSet3Size, fill = 'yellow', bold = True, font='monospace', border = 'black')
...
def onStep(app):
    ...
    #Word increasing and shrinking effect
    ...
    if app.isStarting == True or app.alive == False:
        ...
        #Generate a bunch of sawblades on starting screen :)
        ...
        #Rotating sawblades effect

        app.starAngle -= 20

    #Handling transitions from starting page with a sliding screen effect
    ...
    # "Process of dying" animation
        ...
        if app.isHoldingJump and app.jumps == 0:
            if app.jumpHoldTime < app.maxJumpHoldTime:
                app.jumpHoldTime += 1
                holdFactor = (1 - (app.jumpHoldTime / app.maxJumpHoldTime)) ** 1.65 #ChatGPT assistance for making a longer up arrow key hold result                                                                    
                app.dy = -6.5 - (6.5 * holdFactor)                                  # in a higher jump until a certain height

        #Running and dust animation frames
        ...
        #Jumping animation frames: Calculating the peak of the jump and changing the animation frame speed accordingly
        ...
        #Create "slam" effect when player jumps over sawblade

        if app.scoreSlamActive: 
            if app.scoreSlamTimer < 5:
                app.scoreSize += 4
            elif 5 <= app.scoreSlamTimer < 20:
                app.scoreSize -= 4 
            app.scoreSlamTimer += 1
            if app.scoreSlamTimer >= 10: 
                app.scoreSlamActive = False 
                app.scoreSlamTimer = 0 
                app.scoreSize = 130

        #Gravity and player bounds
        ...
        if app.umbrellaCooldown > 0: 
            app.umbrellaCooldown -= 1

        #Calling sawblade collision functions

        for sawblade in app.sawblades:
            bounceHorizontallySawblade(app, sawblade)
            bounceVerticallySawblade(app, sawblade)
            if app.umbrellaActive:
                umbrellaCollision(app, sawblade)

        #Handling game difficulty: more sawblades allowed on screen as game progresses
        ...
        #Trig calculations for umbrella to rotate around the player and umbrella timer
        ...
        #Special sawblades move toward player
...
#Checking if player jumps over a sawblade and removes it if so
...
#Checking if player jumps over a special sawblade and removes it if so
...
#Distance formula for collision detection etc...
    
def distance(x0, y0, x1, y1):
    return ((x1 - x0)**2 + (y1 - y0)**2)**0.5

def scoreSlam(app):
    app.scoreSlamActive = True 
    app.scoreSlamTimer = 0

#Checking for collisions with sawblades and special sawblades
#Allow some slight overlap without ending game so game isn't too frustrating
...
#Sawblades bounce off the walls
#They disappear when they go off the top of the screen
...
#Sawblades on starting page do not bounce
#They disappear when they go off any part of the screen
...
#Creating a gem at the location of the sawblade that was destroyed
...
#Creating a normal sawblade at a random location, random-ish dx, and a constant dy
...
#Creating normal sawblades in starting screen that travel with a smaller dx
...
#1/150 probability of spawning a special sawblade on any step if there does not already exist one
# only spawns if score is greater than 5, so the game gradually gets harder
...
#Creating special sawblades at random x coordinate
...
#Checks if the umbrella is blocking the line of sight from the
#special sawblade to the player. If it is, then the special sawblade acts as
#any regular sawblade with a random dx. If it is not, it tracks the player
...
def canSee(x1, y1, x2, y2, x3, y3, x4, y4):
    ...
    def inBoundingBox(x, y, xMin, xMax, yMin, yMax): 
        return xMin - 1e-6 <= x <= xMax + 1e-6 and yMin - 1e-6 <= y <= yMax + 1e-6 #Adding minor negligence for bounding box checking
    ...
    x = (b2 * c1 - b1 * c2) / det #ChatGPT assisted me in calculating the (x, y) coordinates
    y = (a1 * c2 - a2 * c1) / det #of the intersection between the umbrella line and the line
                                  #from the special sawblade to the person

    #Check if (x, y) is inside BOTH the box formed by the umbrella line and the box formed by the
    #line from the special sawblade to the person
...
#Creating classes for sawblades and gems
...
#Creating class for special sawblade
...
class specialSawblade:
    ...
    #If the umbrella blocks the line of sight or umbrella doesn't exist, the special sawblade acts as a normal sawblade with random dx
    ...
    def moveTowardsPlayer(self, app, playerX, playerY):
        ...
        if dist != 0:
            if app.umbrellaActive:
                if not canSee(self.xCoord, self.yCoord, playerX, playerY, app.lineX1, app.lineY1, app.lineX2, app.lineY2) and app.umbrellaCooldown <= 0:
                    app.canSee = False
                    if self.tracking:
...
#Handles bouncing off walls for special sawblades
...
def bounceVerticallySpecialSawblade(app, specialSawblade): 
    ...
    if specialSawblade.yCoord + specialSawblade.radius >= app.backgroundY + app.backgroundHeight: 
        ...
        specialSawblade.DY = -specialSawblade.DY
        
        for specialSawblade in app.specialSawblade: 
            if specialSawblade.yCoord + specialSawblade.radius + 100 <= 0: 
                app.specialSawblade.remove(specialSawblade)

#Checked my math below with Tanvi Chordiya, a student at Green Level High School in North Carolina

def umbrellaCollision(app, sawblade):
    r = 50  # Radius of the theoretical circle of which the umbrella is an arc
    cx, cy = app.cx, app.cy
    sx, sy = sawblade.xCoord, sawblade.yCoord

    distanceToCenter = distance(cx, cy, sx, sy) # Calculate distance to umbrella center
    
    if distanceToCenter < r + sawblade.radius and app.umbrellaCooldown <= 0: # Check if sawblade collides with theoretical circle

        if sy < app.lineY1 + 10: # Increasing the hitbox range slightly

            if not canSee(sx, sy, cx, cy, app.lineX1, app.lineY1, app.lineX2, app.lineY2): # Checking if the sawblade collides with the umbrella itself

                # Calculating the normal vector at the point of collision with umbrella
                                  
                normalX = sx - cx
                normalY = sy - cy
                normalLength = math.sqrt(normalX ** 2 + normalY ** 2) 
                ...
                # Reflect the sawblade's velocity over the normal vector

                dotProduct = sawblade.sawbladeDX * normalX + sawblade.sawbladeDY * normalY
                sawblade.sawbladeDX = sawblade.sawbladeDX - 2 * dotProduct * normalX
                sawblade.sawbladeDY = sawblade.sawbladeDY - 2 * dotProduct * normalY

                sawblade.xCoord += normalX
                sawblade.yCoord += normalY

                app.umbrellaCooldown = 10 #Prevents minor glitches with umbrella bouncing my having bounces only work after intervals of 10
